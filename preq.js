...
document.addEventListener("input", () => {
  ...
  const field6 = !!document.querySelectorAll(".date-field").value;
  ...
  const handleCheckboxClicked = (elem) => {
    if (elem.target.value && (field5 || field4)) {
      document.getElementById("nextb2").style.display = "block";
      document.getElementById("continueb2").style.display = "none";
    }
  };

  const checkboxElems = document.querySelectorAll(".date-field");
  for (let i = 0; i < checkboxElems.length; i++) {
    checkboxElems[i].addEventListener("input", handleCheckboxClicked);
  }
});
...
{
  var datesCollection = document.getElementsByClassName("date-field");
...
{
  var datesCollection = document.getElementsByClassName("date-field2");
  var dates = Array.from(datesCollection);
